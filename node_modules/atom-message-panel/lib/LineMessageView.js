...
var LineMessageView = function (params) {
  ...
  this.color = params.color || "#dddddd";
...
LineMessageView.content = function () {
  this.div({
    ...
    this.pre({
      ...
      outlet: 'code',
      click: 'goToLine',
      style: 'cursor: pointer; margin-left: 8px; margin-top: 8px; margin-bottom: 8px;'
    });
  }.bind(this));
};
...
LineMessageView.prototype.initialize = function () {
  var message = '<span style="color: ' + this.color + ';">at line ' + this.line;
  if (this.character !== undefined) {
    message += ', character ' + this.character;
  }
  if (this.file !== undefined) {
    message += ', file ' + this.file + '</span>';
  }
  this.position.html(message);
  this.contents.html(this.message);
  if (this.className) {
    this.contents.addClass(this.className);
  }

  if (this.preview) {
    this.code.html(this.preview);
  } else {
    this.code.remove();
  }
};
